...
const { getRoom, deleteRoom, rooms, userRoomMap, removeUserFromRoom } = require("./roomStore");
const {
  createTournament,
  generateMatches,
  getNextMatch,
...
function handleJoinRoom(ws, data) {
  ...
  if (room.participants.some(p => p.pseudo === pseudo)) {
...
function startGameLoop(room) {
  ...
  let remaining = 0;
  if (!room.gameStarted) {
    const elapsed = (now - room.countdownStart) / 1000;
    remaining = room.countdownSeconds - Math.floor(elapsed);
    ...
    ["player1", "player2"].forEach(role => {
      ...
      if (p?.ws.readyState === WebSocket.OPEN) {
        p.ws.send(JSON.stringify({
          ...
          state: { ball: room.ball, paddles: room.paddles,
    ...
    ["player1","player2"].forEach(role => {
      const p = room.players[role];
      if(p?.ws.readyState===WebSocket.OPEN) p.ws.send(JSON.stringify({ type:"state", gameStarted: true, state:{ball:room.ball,paddles:room.paddles,scores:{player1:room.paddles.player1.score,player2:room.paddles.player2.score}}}));
    });
  }, 1000 / TICK_RATE);
}
