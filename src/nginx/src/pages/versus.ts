...
export function versusContent(): HTMLElement {
  ...
  start.addEventListener('click', async () => {
    ...
    if (!GameClass) { alert('Le jeu n\'a pas été chargé.'); return; }
    ...
    ws.onmessage = (e) => {
      ...
      if (data.type === "assign") playerRole = data.player;
      else if (data.type === "input") state.currentGame?.moveRemotePaddle(data.key, true);
      else if (data.type === "input-up") state.currentGame?.moveRemotePaddle(data.key, false);
      else if (data.type === "gameOver") renderVictory(data.winner, data.loser, data.score, a, b);
    };
