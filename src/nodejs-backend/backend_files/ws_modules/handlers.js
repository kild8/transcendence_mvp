...
function handleJoinRoom(ws, data) {
  ...
  if (room.disconnectedPlayer && room.disconnectedPlayer.pseudo === pseudo) {
	...
	//if is reconnecting, clear the timeout
	...
	//notfity other player of the reconnection
  ...
  //set the host to the first player
...
function handleStartTournament(ws, data) {
  ...
  //set tournament to started state
  ...
  //waiting the host to start the match
  room.tournament.waitingForHost = true;
  //display the match overview/matchups
  ...
  //set up the parameters, position of the game
...
//start a whole round of a tournament, notify the players, and triggers next match
...
//receive the players input and stock them in lastInputs array
...
//handle the disconnect of a player during a game, keep the state, notify players, if more than 10s, set the gameover and win to the standing player
function handleDisconnect(ws) {
	//return if the socket is not a playing socket
	if (ws.socketRole === 'lobby') return;
	//get the players
  Object.values(rooms).forEach(room => {
    const idx = room.participants.findIndex(p => p.ws === ws);
    if (idx === -1) return;
	//remove the disconnected player from the participants array
    const disconnectedPlayer = room.participants[idx];
    room.participants.splice(idx, 1);
	//if the host is disconnected, update the host to next player in array and notify the players
    ...
    if (role) {
	  ...
	  //notify other participants of the disconnection
      room.participants.forEach(p => {
        if (p.ws.readyState === WebSocket.OPEN) {
	  ...
	  //run a 10s timeout and declare a loser if exceeds
      room.disconnectTimeout = setTimeout(() => {
		...
		//delete room if no more participants
